package com.example.vasux.receiver;
...
import static com.example.vasux.receiver.ReceiverActivity.WifiTasksHandler.SCAN_FOR_WIFI_RESULTS;
import static com.example.vasux.receiver.ReceiverActivity.WifiTasksHandler.WAIT_FOR_CONNECT_ACTION_TIMEOUT;
import static com.example.vasux.receiver.ReceiverActivity.WifiTasksHandler.WAIT_FOR_RECONNECT_ACTION_TIMEOUT;
...
public class ReceiverActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (Utils.isShareServiceRunning(getApplication())) {
            Toast.makeText(this, "Transfer mode is active, stop Transfer service to proceed with Receiving files", Toast.LENGTH_SHORT).show();
            return;
        }
    ...
    public void onResume() {
        super.onResume();
        boolean isConnectedToVasuXAp = WifiUtils.isWifiConnectedToSTAccessPoint(getApplicationContext());
        if (isConnectedToVasuXAp) {
            unRegisterForScanResults();
            if (!m_receiver_control.isChecked())
                changeReceiverControlCheckedStatus(true);
            String ssid = wifiManager.getConnectionInfo().getSSID();
            Log.d(TAG, "Wifi is connected/connecting to VasuX ap, ssid: " + ssid);
            mConnectedSSID = ssid;
            addSenderFilesListingFragment(WifiUtils.getAccessPointIpAddress(this), ssid);
        } else if (m_receiver_control.isChecked()) {
            Log.d(TAG, "wifi is not connected to VasuX ap, initiating sender search..");
            resetSenderSearch();
        }
    }
    ...
    private void connectToWifi(String ssid) {
        ...
        if (info.getSSID().equals(ssid)) {
            Log.d(TAG, "Already connected to VasuX, add sender Files listing fragment");
            resetWifiScan = false;
            ...
            resetWifiScan = !WifiUtils.connectToOpenWifi(wifiManager, ssid, false);
            Log.e(TAG, "connection attempt to VasuX wifi is " + (!resetWifiScan ? "success!!!" : "FAILED..!!!"));
        }
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals(WifiManager.SCAN_RESULTS_AVAILABLE_ACTION) && !WifiUtils.isWifiConnectedToSTAccessPoint(getApplicationContext())) {
                ...
                for (ScanResult result : mScanResults)
                    if (WifiUtils.isVasuXSSID(result.SSID) && (isOreoOrAbove() || WifiUtils.isOpenWifi(result))) {
                ...
                if (ConnectivityManager.TYPE_WIFI == netInfo.getType()) {
                    ...
                    if (WifiUtils.isVasuXSSID(info.getSSID())) {
                        if (System.currentTimeMillis() - preferences.getLong(LASTCONNECTEDTIME, 0) >= SYNCTIME && supState.equals(SupplicantState.COMPLETED)) {
                            ...
                            Log.d(TAG, "client connected to VasuX hot spot. AP ip address: " + ip);
