...
import com.example.vasux.utils.WifiUtils;
...
import static com.example.vasux.sender.VasuXActivity.ShareUIHandler.LIST_API_CLIENTS;
import static com.example.vasux.sender.VasuXActivity.ShareUIHandler.UPDATE_AP_STATUS;
import static com.example.vasux.utils.Utils.DEFAULT_PORT_OREO;
import static com.example.vasux.utils.Utils.getRandomColor;
import static com.example.vasux.utils.Utils.isOreoOrAbove;


public class VasuXActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (null != getIntent() && getIntent().hasExtra(VasuXService.EXTRA_FILE_PATHS))
            m_sharedFilePaths = getIntent().getStringArrayExtra(VasuXService.EXTRA_FILE_PATHS);
                ...
                if (isChecked) {
                    if (!isOreoOrAbove()) {
                        ...
                        if (Build.VERSION.SDK_INT >= 23 &&
                                ...
                                Utils.getTargetSDKVersion(getApplicationContext()) >= 23 && !Settings.System.canWrite(VasuXActivity.this)) {
        ...
        m_p2pServerUpdatesListener = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                if (isFinishing() || null == intent)
                    return;
                int intentType = intent.getIntExtra(VasuXService.ShareIntents.TYPE, 0);
                if (intentType == VasuXService.ShareIntents.Types.FILE_TRANSFER_STATUS) {
                    String fileName = intent.getStringExtra(VasuXService.ShareIntents.SHARE_SERVER_UPDATE_FILE_NAME);
                    updateReceiverListItem(intent.getStringExtra(VasuXService.ShareIntents.SHARE_CLIENT_IP), intent.getIntExtra(VasuXService.ShareIntents.SHARE_TRANSFER_PROGRESS, -1), intent.getStringExtra(VasuXService.ShareIntents.SHARE_SERVER_UPDATE_TEXT), fileName);
                } else if (intentType == VasuXService.ShareIntents.Types.AP_DISABLED_ACKNOWLEDGEMENT) {
                    shouldAutoConnect = false;
                    resetSenderUi(false);
                }
            }
        };
        registerReceiver(m_p2pServerUpdatesListener, new IntentFilter(VasuXService.ShareIntents.SHARE_SERVER_UPDATES_INTENT_ACTION));
    }
    ...
    private void disableAp() {
        ...
        Intent p2pServiceIntent = new Intent(getApplicationContext(), VasuXService.class);
        p2pServiceIntent.setAction(VasuXService.WIFI_AP_ACTION_STOP);
    ...
    private void startP2pSenderWatchService() {
        Intent p2pServiceIntent = new Intent(getApplicationContext(), VasuXService.class);
        p2pServiceIntent.putExtra(VasuXService.EXTRA_FILE_PATHS, m_sharedFilePaths);
        if (null != getIntent()) {
            p2pServiceIntent.putExtra(VasuXService.EXTRA_PORT, isOreoOrAbove() ? DEFAULT_PORT_OREO : getIntent().getIntExtra(VasuXService.EXTRA_PORT, 0));
            p2pServiceIntent.putExtra(VasuXService.EXTRA_SENDER_NAME, getIntent().getStringExtra(VasuXService.EXTRA_SENDER_NAME));
        }
        p2pServiceIntent.setAction(VasuXService.WIFI_AP_ACTION_START);
        startService(p2pServiceIntent);
    }

    private void startHostspotCheckOnService() {
        Intent p2pServiceIntent = new Intent(getApplicationContext(), VasuXService.class);
        p2pServiceIntent.setAction(VasuXService.WIFI_AP_ACTION_START_CHECK);
        startService(p2pServiceIntent);
    }
...
//            m_receiversListAdapter.remove(new WifiApControl.Client(ip, null, null));
    ...
    static class ShareUIHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            VasuXActivity activity = mActivity.get();
