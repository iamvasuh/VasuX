...
import static com.example.vasux.sender.VasuXService.ShareIntents.SHARE_CLIENT_IP;
import static com.example.vasux.sender.VasuXService.ShareIntents.SHARE_SERVER_UPDATES_INTENT_ACTION;
import static com.example.vasux.sender.VasuXService.ShareIntents.SHARE_SERVER_UPDATE_FILE_NAME;
import static com.example.vasux.sender.VasuXService.ShareIntents.SHARE_SERVER_UPDATE_TEXT;
import static com.example.vasux.sender.VasuXService.ShareIntents.SHARE_TRANSFER_PROGRESS;
import static com.example.vasux.sender.VasuXService.ShareIntents.TYPE;
import static com.example.vasux.sender.VasuXService.ShareIntents.Types.AP_DISABLED_ACKNOWLEDGEMENT;
import static com.example.vasux.sender.VasuXService.ShareIntents.Types.FILE_TRANSFER_STATUS;
import static com.example.vasux.utils.Utils.isOreoOrAbove;

public class VasuXService extends Service {

    private static final String TAG = "ShareService";

    private WifiManager wifiManager;
    private HotspotControl hotspotControl;
    private VasuXServer m_fileServer;
    ...
    public void onCreate() {
        ...
        startForeground(SHARE_SERVICE_NOTIFICATION_ID, getVasuXNotification(getString(R.string.p2p_sender_service_init_notification_header), false));
...
//                    sendAcknowledgementBroadcast(AP_ENABLED_ACKNOWLEDGEMENT);
     ...
     * @param addStopAction if true STOP action is added to stop {@link VasuXService}
     * @return Notification
     */
    private Notification getVasuXNotification(String text, boolean addStopAction) {
    ...
    /**
     * Starts {@link VasuXServer} and then enables Hotspot with assigned port being part of encoded SSID.
     *
    ...
    private void startFileHostingServer(String[] filePathsTobeServed, int port, final String sender_name) {
        m_fileServer = new VasuXServer(getApplicationContext()
...
//        }).start();
    ...
    private void updateForegroundNotification() {
        ...
        mNotificationManager.notify(SHARE_SERVICE_NOTIFICATION_ID, getVasuXNotification(getString(R.string.p2p_sender_service_notification_header), true));
    }

    private static class HotspotChecker extends Handler {
        WeakReference<VasuXService> service;

        HotspotChecker(VasuXService senderService) {
            this.service = new WeakReference<>(senderService);
        }

        @Override
        public void handleMessage(Message msg) {
            VasuXService senderService = service.get();
